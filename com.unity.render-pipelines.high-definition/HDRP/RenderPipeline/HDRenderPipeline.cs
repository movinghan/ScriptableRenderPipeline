...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    public class HDRenderPipeline : RenderPipeline
    {
                ...
                using (new ProfilingSample(cmd, "HDRenderPipeline::Render", CustomSamplerId.HDRenderPipelineRender.GetSampler()))
                {
                    ...
                    else
                    {
                        ...
                        if (m_LightLoop.GetFeatureVariantsEnabled())
                        {
                            ...
                            {
                                HDUtils.SetRenderTarget(cmd, hdCamera, m_SharedRTManager.GetDepthStencilBuffer(), ClearFlag.Color, CoreUtils.clearColorAllBlack);
        ...
        void RenderSSAO(CommandBuffer cmd, HDCamera hdCamera, ScriptableRenderContext renderContext, PostProcessLayer postProcessLayer, FrameSettings frameSettings)
        {
            ...
            if (hdCamera.frameSettings.enableSSAO && postProcessLayer != null && postProcessLayer.enabled)
            {
                ...
                if (settings.IsEnabledAndSupported(null))
                {
                    ...
                    {
                        postProcessLayer.BakeMSVOMap(cmd, camera, m_AmbientOcclusionBuffer, m_SharedRTManager.GetDepthStencilBuffer(), true, false);
                    }
        ...
        void RenderDepthPyramid(HDCamera hdCamera, CommandBuffer cmd, FullScreenDebugMode debugMode)
        {
            ...
            {
                var size = new Vector2Int(hdCamera.actualWidth, hdCamera.actualHeight);
                lodCount = m_MipGenerator.RenderMinDepthPyramid(cmd, size, m_SharedRTManager.GetDepthTexture());
            }
            ...
            PushFullScreenDebugTextureMip(hdCamera, cmd, m_SharedRTManager.GetDepthTexture(), lodCount, new Vector4(scaleX, scaleY, 0f, 0f), debugMode);
        }
